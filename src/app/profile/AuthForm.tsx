    ...
    <div className="max-w-md mx-auto">
      ...
      <div className="bg-nom8-card rounded-xl border border-white/5 p-6 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-nom8-text mb-1">Password</label>
            <input
              type="password"
              value={password}
        ...
        <p className="text-center text-sm text-nom8-text-muted">
          {mode === "login" ? (
            <>Don&apos;t have an account?{" "}
              <button type="button" onClick={() => { setMode("register"); setError(""); }} className="text-nom8-orange hover:underline">Sign up</button>
            </>
          ) : (
            <>Already have an account?{" "}
              <button type="button" onClick={() => { setMode("login"); setError(""); }} className="text-nom8-orange hover:underline">Sign in</button>
            </>
          )}
        </p>
      </div>
